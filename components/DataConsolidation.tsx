...
app.use(express.json({ limit: '50mb' })); // Increased limit significantly for large imports
...
const isAdmin = async (req, res, next) => {
    ...
    if (!username) {
        console.warn("[AUTH FAIL] No username provided in request:", req.path);
        return res.status(401).json({ message: "Authentication required" });
    }

    // Trim whitespace to avoid "admin " != "admin" issues
    username = String(username).trim();

    try {
        const [rows] = await db.promise().query('SELECT role FROM users WHERE username = ?', [username]);
        if (rows.length === 0) {
            console.warn(`[AUTH FAIL] User not found: ${username}`);
            return res.status(403).json({ message: "Access denied. User not found." });
        }
        
        if (rows[0].role !== 'Admin') {
            console.warn(`[AUTH FAIL] User ${username} has role ${rows[0].role}, expected Admin.`);
            return res.status(403).json({ message: "Access denied. Admin privileges required." });
        }
        
        req.userRole = rows[0].role;
        // Update the trimmed username back to body/query if possible, or just use it for validation
...
app.post('/api/equipment/import', isAdmin, async (req, res) => {
    ...
    if (!equipmentList || !Array.isArray(equipmentList)) {
        return res.status(400).json({ success: false, message: "equipmentList must be an array" });
    }
